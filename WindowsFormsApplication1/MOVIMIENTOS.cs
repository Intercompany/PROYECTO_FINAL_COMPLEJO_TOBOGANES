...
namespace WindowsFormsApplication1
{
    public partial class MOVIMIENTOS : Form
    {
        ...
        public string mensaje = "INGRESE CODIGO DE AUTORIZACION";
        ...
        void FILTRAR_CAJA_KARDEX(int OPCION, string VER, string DESCRIPCION)
        {
            ...
            TOTAL_SOLES = 0.00;
            TOTAL_DOLARES = 0.00;
            //cboTIPO_BUSQUEDA.SelectedIndex = 1;
            ...
            dgvMOV_CAJAKARDEX.Columns[20].HeaderText = "ID_VENTA";
        ...
        void BUSQUEDA_COMVENT()
        {
            ...
            string ID_MOVIMIENTO = cboTIPO_BUSQUEDA.SelectedValue.ToString();
            string ID_CAJA = Properties.Settings.Default.id_caja;
            string TIPO_PAGO = string.Empty;
            string ID_TIPOMOV = string.Empty;
            string OPCION_USUARIO = string.Empty;
            ...
            con.Open();
            SqlCommand cmd = new SqlCommand("SELECT ID_MOVIMIENTO, DESCRIPCION, TP_DESCRIPCION, ID_TIPOMOV, TM_DESCRIPCION, IMPORTE, MONEDA,"+
                                        "TIPO_CAMBIO, AMORTIZADO, IMPORTE_CAJA, ID_CAJA, FECHA_INICIAL, FECHA_CIERRE, SALDO_INICIAL, SALDO_FINAL, EMPLEADO,"+
                                        "PV_DESCRIPCION, S_DESCRIPCION, FECHA_ANULADO, FECHA, ID_COMPVENT FROM V_CAJA_KADEX" +
                                        " WHERE ID_CAJA = '"+ ID_CAJA + "' AND ID_COMPVENT = '"+txtDATA_BUSQUEDA.Text+"' AND FECHA_ANULADO IS NULL ",con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dat = new DataTable();
            da.Fill(dat);

            dgvMOV_CAJAKARDEX.DataSource = dat;
            ...
            {
                TOTAL_CAJA = TOTAL_CAJA + Convert.ToDouble(dat.Rows[i]["IMPORTE_CAJA"]);
                if (dat.Rows[i]["MONEDA"].ToString() == "S")
                {
                    if (dat.Rows[i]["ID_TIPOMOV"].ToString().Substring(0, 1) == "I")
                    {
                        TOTAL_SOLES = TOTAL_SOLES + Convert.ToDouble(dat.Rows[i]["IMPORTE"]);
                    }
                    else
                    {
                        TOTAL_SOLES = TOTAL_SOLES - Convert.ToDouble(dat.Rows[i]["IMPORTE"]);
                ...
                else
                {
                    if (dat.Rows[i]["ID_TIPOMOV"].ToString().Substring(0, 1) == "I")
                    {
                        TOTAL_DOLARES = TOTAL_DOLARES + Convert.ToDouble(dat.Rows[i]["IMPORTE"]);
                    }
                    else
                    {
        ...
        void LLENAR_COMBO_BUSQUEDA()
        {
            
            List<ListaTipoProd> List = new List<ListaTipoProd>();
            List.Add(new ListaTipoProd { texto = "TODOS", value = "0" });
            List.Add(new ListaTipoProd { texto = "ID MOVIMIENTO", value = "1" });
            List.Add(new ListaTipoProd { texto = "ID_VENTA", value = "2" });
            cboTIPO_BUSQUEDA.ValueMember = "value";
            cboTIPO_BUSQUEDA.DisplayMember = "texto";
            cboTIPO_BUSQUEDA.DataSource = List;
        ...
        {
            if (cboTIPO_MOV.Text != "INGRESO POR VENTA" && cboTIPO_MOV.Text != "EGRESO POR COMPRA")
            {
        ...
        public void ShowMyDialogBoxIMP(string mensaje)
        {
        ...
        {
            if (dgvMOV_CAJAKARDEX.Rows.Count != 0 && rdbACTIVOS.Checked == true && rdbTODOS.Checked == false && rdbANULADOS.Checked == false ) //AQUI VALIDO QUE EXISTAN DATOS EN MI GRIDVIEW PARA PODER IMPRIIMIR MIS DATOS
            {

                if (dgvMOV_CAJAKARDEX.CurrentRow.Cells[3].Value.ToString() != "IPV") //AQUI GENERO EL ARCHIVO PDF PARA SU IMPRESION
                {
                    ShowMyDialogBoxIMP(mensaje);
                    //P_IMPRIMIR_GRABAR();
        ...
        private void btnBUSCAR_Click(object sender, EventArgs e)
        {
            ...
            {
                if (cboTIPO_BUSQUEDA.SelectedValue.ToString() == "2")
                {
                    BUSQUEDA_COMVENT();
                } else { 
                    FILTRAR_CAJA_KARDEX(Convert.ToInt32(cboTIPO_BUSQUEDA.SelectedValue.ToString()), "1", txtDATA_BUSQUEDA.Text.ToString());
                }
            }
            if (rdbANULADOS.Checked == true)//ANULADOS
            {
                ...
                {
                    FILTRAR_CAJA_KARDEX(Convert.ToInt32(cboTIPO_BUSQUEDA.SelectedValue.ToString()), "2", txtDATA_BUSQUEDA.Text.ToString());
                }
            }
            if (rdbTODOS.Checked == true)//TODOS
            {
                ...
                {
                    FILTRAR_CAJA_KARDEX(Convert.ToInt32(cboTIPO_BUSQUEDA.SelectedValue.ToString()), "3", txtDATA_BUSQUEDA.Text.ToString());
                }
            }
        }
        ...
        void IMPRIMIR_SPOOL_TODOS_MOVCAJA()
        {
            ...
            Ticket1.TextoCentro("FECHA APERTURA: " + Properties.Settings.Default.fecha_apertura_caja);
            Ticket1.TextoIzquierda("FECHA CIERRE: " + DateTime.Now.ToString());
            Ticket1.LineasGuion(); // imprime una linea de guiones
            ...
            Ticket1.TextoCentro("");
            Ticket1.TextoCentro("TOTAL ANULADOS: " + CONTANU + " DOC ");
        ...
        private void txtCODANULACION_TextChanged(object sender, EventArgs e)
        {
        ...
        private void txtDATA_BUSQUEDA_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (Char.IsDigit(e.KeyChar))
            {
                e.Handled = false;
        ...
        private void txtDATA_BUSQUEDA_TextChanged(object sender, EventArgs e)
        {
            //FILTRAR_CAJA_KARDEX(2,"1",txtDATA_BUSQUEDA.Text);
        }
    }
}
